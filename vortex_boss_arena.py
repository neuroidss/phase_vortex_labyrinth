...
# =====================================================================
# GLOBAL COMBAT BALANCE & POWER RATING CONFIGURATION
# =====================================================================
...
COMBAT_BALANCE_CONFIG = {
    ...
    # --- HEALING BALANCE & ANTI-IMMORTALITY ---
    # Multiplier applied when a unit heals itself (0.35 efficiency prevents immortal healers)
    "SELF_HEAL_EFFICIENCY": 0.35,
    # Progressive anti-heal dampening per second of match time: 1.0 / (1.0 + time * dampening)
    "HEAL_DAMPENING_PER_SEC": 0.012,
    # Base HP restoration rate when absorbing 6Hz Theta mist
    "BASE_HEAL_RATE": 0.038,
    # Theta mist density generated per second by channel_heal
    "HEAL_MIST_CHANNEL_POWER": 15.0,
    # Theta mist density generated by ranged heal soliton
    "HEAL_SOLITON_POWER": 1.2,

    # --- DAMAGE & SHIELD PHYSICS ---
    # Base multiplier for Gamma spectral damage
    "DAMAGE_SCALE_BASE": 0.085,
    # Percentage of damage passing through an active 10Hz Alpha shield field
    "SHIELD_PENETRATION_RATIO": 0.15,
    # Density of 10Hz Alpha shield generated by Shield channel
    "SHIELD_CHANNEL_POWER": 45.0,
}
...
class SoftbodyActor:
    def __init__(self, spawn_pos, team, color, name, style, weapon, creature, level, quality):
        ...
        w_cfg = WEAPON_CONFIGS.get(self.weapon, {"cooldown_min": 0.5, "cooldown_max": 1.0})
        self.cooldown = random.uniform(w_cfg["cooldown_min"], w_cfg["cooldown_max"])
        self.channel_duration = 0.0
        self.channel_target = None
        self.power = self.calculate_power()

    def calculate_power(self):
        """ Dynamically computes actor power from level, quality, weapon DPS weights, and creature stats """
        ...
        w_cfg = WEAPON_CONFIGS.get(self.weapon, {"power_mult": 1.0, "dps_weight": 1.0})
        c_cfg = CREATURE_CONFIGS.get(self.creature, {"power_mult": 1.0, "hp_mult": 1.0, "speed_mult": 1.0})
        
        weapon_impact = w_cfg["power_mult"] * (0.7 + 0.3 * w_cfg["dps_weight"])
...
class BossFightArena:
    ...
    def step(self, dt):
        ...
        # --- TARGET ABSORPTION & HEALING (ANTI-IMMORTALITY BALANCE) ---
        ...
        match_heal_dampening = 1.0 / (1.0 + self.combat_time * COMBAT_BALANCE_CONFIG["HEAL_DAMPENING_PER_SEC"])
        ...
        for act in self.actors:
            ...
            for i in range(16):
                ...
                if 0 <= gx < COMPUTE_RES and 0 <= gy < COMPUTE_RES:
                    ...
                    # Balanced healing with self-heal penalty and match time dampening
                    if theta_val > 0.25 and act.integrity < 1.0:
                        is_self_heal = (act.weapon == "Heal" and act.current_action.startswith("[HEALING SELF]"))
                        self_heal_mult = COMBAT_BALANCE_CONFIG["SELF_HEAL_EFFICIENCY"] if is_self_heal else 1.0
                        
                        heal_rate = theta_val * COMBAT_BALANCE_CONFIG["BASE_HEAL_RATE"] * self_heal_mult * match_heal_dampening
        ...
        for act in self.actors:
            ...
            w_cfg = WEAPON_CONFIGS.get(act.weapon, {"cooldown_min": 1.0, "cooldown_max": 2.0})
                ...
                if act.weapon == "Magic":
                    ...
                    if act.channel_duration > 0:
                        ...
                        if act.cooldown <= 0:
                            ...
                            act.cooldown = random.uniform(w_cfg["cooldown_min"], w_cfg["cooldown_max"])
                    ...
                    if act.cooldown <= 0:
                        ...
                        act.cooldown = random.uniform(w_cfg["cooldown_min"], w_cfg["cooldown_max"]) if success else 0.1
